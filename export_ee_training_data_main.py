...
FLAGS = flags.FLAGS
flags.DEFINE_string('bucket', 'hru-fire', 'Output file bucket on GCP.')
flags.DEFINE_string('folder', 'tmp', 'Output folder path under the file bucket.')
flags.DEFINE_string('start_date', '2020-01-01', 'Start date to export in YYYY-MM-DD.')
flags.DEFINE_string('end_date', '2021-01-01', 'End date to export in YYYY-MM-DD (inclusive).')
flags.DEFINE_string('prefix', '', 'File prefix for output files.')
flags.DEFINE_string('config_dir', '', 'If non-empty, a JSON file of flag values is written to this directory.')
flags.DEFINE_integer('kernel_size', 64, 'Size of the exported tiles in pixels (square).')
flags.DEFINE_integer('sampling_scale', 1000, 'Resolution at which to export the data (in meters).')
flags.DEFINE_float('eval_split_ratio', 0.1, 'Evaluation dataset as a proportion of the total dataset.')
flags.DEFINE_integer('num_samples_per_file', 2000, 'Approximate number of samples per tf.Examples file.')
...
def main(argv):
  ...
  # if yes, save the above flags in a json file
  if FLAGS.config_dir:
