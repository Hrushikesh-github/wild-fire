...
def _get_all_feature_bands():
  ...
  return (ee_utils.DATA_BANDS[ee_utils.DataType.ELEVATION_SRTM] +
          ...
          ['PrevFireMask'] +
          ee_utils.DATA_BANDS[ee_utils.DataType.LST] +
          ee_utils.DATA_BANDS[ee_utils.DataType.LULC])
...
def _get_all_image_collections():
  ...
  image_collections = {
      ...
      'LST':
          ee_utils.get_image_collection(ee_utils.DataType.LST),
      'LULC':
          ee_utils.get_image_collection(ee_utils.DataType.LULC)
  }
  time_sampling = {
      'drought':
      ...
      'LST':
          ee_utils.DATA_TIME_SAMPLING[ee_utils.DataType.LST],
      'LULC':
          ee_utils.DATA_TIME_SAMPLING[ee_utils.DataType.LULC],
  }
...
def _get_time_slices(
  ...
  LST = image_collections['LST'].filterDate(window_start, window_end).map(
      ee_utils.remove_mask).max()
  return [drought, vegetation, weather, prev_fire, fire, detection, LST]
...
def _export_dataset(
  ...
  LULC = ee_utils.get_image(ee_utils.DataType.LULC)
  ...
  for start_day in all_days:
    ...
    time_slices = _get_time_slices(window_start, window, projection,
                                   resampling_scale)
    image_list = [elevation, population, LULC] + time_slices[:-1]
